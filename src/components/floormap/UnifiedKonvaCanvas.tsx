...
import { getUnifiedObjectById, UnifiedObjectDefinition, isElectricalShape } from './objectLibrary';
      ...
      >
        <Layer>
          ...
          {(() => {
            ...
            // El-filter: hide placed electrical fixtures when toggled off.
            const nonRoomShapes = sortedShapes.filter(
              s => s.type !== 'room' &&
                !(simplified && (s.type === 'wall' || s.type === 'line') && s.metadata?.autoGenerated) &&
                (projectSettings.showElectrical || !isElectricalShape(s))
            );
