-- Single-source the demo quote line items into seed_demo_content.
--
-- The prior fix (20260817170000) backfilled quote_items for the PUBLIC demo only.
-- This folds the same 12 lines into seed_demo_content itself so per-user demos and
-- any future rebuild also get them — the canonical source. Lines mirror the tasks
-- (customer price ex VAT, sum 134 000; per-task ROT, sum 21 120). The public demo
-- already has these lines (20260817170000); this only updates the function for
-- future per-user demos + rebuilds — no destructive re-seed here.
