-- SP2: Demo floor plan — derive walls + doors from the room polygons so the
-- Space tab opens on a finished-looking drawing instead of an empty canvas.
--
-- Background: seed_demo_content seeds room polygons as LEGACY shapes
-- (plan_id IS NULL) which the plan-scoped editor never loads — the demo's
-- "Floor Plan 1" (auto-created, empty) rendered a blank grid. The room layout
-- was designed with exact 200 mm gaps between polygons = the wall pockets:
--   Kök (0,0)-(4000,2000) · Badrum (4200,0)-(6200,2000)
--   Hall (0,2200)-(5000,3300) · Vardagsrum (0,3500)-(5000,7500)
--   Sovrum (5200,2500)-(8200,6500)
-- Walls are centered in the gaps (interior) and 100 mm outside the envelope
-- (exterior), thickness 200 → wall faces sit flush with the room edges.
--
-- Single-source: new function seed_demo_floor_plan(project, language) is
-- called from seed_demo_content (per-user demos + rebuilds) AND invoked once
-- here to backfill the PUBLIC demo. Idempotent: clears plan-scoped shapes
-- before re-inserting; legacy room rows are left untouched (room views use
-- them). Shape JSON mirrors saveShapesForPlan/loadShapesForPlan exactly
-- (walls: coordinates{x1,y1,x2,y2}+thicknessMM; doors: library symbols via
-- metadata.symbolType — same contract as the foto→plan pipeline emits).
...
BEGIN
  ...
  -- Find or create the plan; normalize its name and make it default.
  ...
  -- Idempotent: clear previous plan-scoped shapes (legacy plan_id-NULL rows stay).
  DELETE FROM floor_map_shapes WHERE project_id = p_project_id AND plan_id = v_plan_id;

  -- Plan-scoped copies of the room polygons (verbatim shape_data, keeps room_id).
  INSERT INTO floor_map_shapes (id, project_id, room_id, plan_id, shape_type, shape_data)
  SELECT gen_random_uuid(), project_id, room_id, v_plan_id, shape_type, shape_data
  FROM floor_map_shapes
  WHERE project_id = p_project_id AND plan_id IS NULL AND shape_type = 'room';

  -- Walls (centerlines in mm; thickness 200, height 2400, ink stroke)
  ...
  FROM (VALUES
    ...
    ( -100,  -100,  6300,  -100, true),   -- north (kök + badrum)
    ( 6300,  -100,  6300,  2400, true),   -- east upper (badrum)
    ( 6300,  2400,  8300,  2400, true),   -- sovrum north
    ( 8300,  2400,  8300,  6600, true),   -- east (sovrum)
    ( 8300,  6600,  5100,  6600, true),   -- sovrum south
    ( 5100,  6600,  5100,  7600, true),   -- east lower (vardagsrum)
    ( 5100,  7600,  -100,  7600, true),   -- south (vardagsrum)
    ( -100,  7600,  -100,  -100, true),   -- west
    -- Interior partitions (centered in the 200 mm polygon gaps)
    ( 4100,  -100,  4100,  2100, false),  -- kök | badrum
    ( -100,  2100,  6300,  2100, false),  -- kök/badrum | hall
    ( -100,  3400,  5100,  3400, false),  -- hall | vardagsrum
    ( 5100,  2100,  5100,  6600, false)   -- hall/vardagsrum | sovrum
  ) AS w(x1, y1, x2, y2, ext);

  -- Doors (library symbols; placement on the wall centerline; rotation picks swing)
  INSERT INTO floor_map_shapes (id, project_id, plan_id, shape_type, shape_data)
  ...
  FROM (VALUES
    ( -100, 2750,  90, 'door_swing_right', 'Entrédörr',      'Entry door'),
    ( 1200, 2100,   0, 'door_swing_left',  'Dörr kök',       'Kitchen door'),
    ( 4700, 2100,   0, 'door_swing_left',  'Dörr badrum',    'Bathroom door'),
    ( 2500, 3400,   0, 'door_swing_left',  'Dörr vardagsrum','Living room door'),
    ( 5100, 2900, 270, 'door_swing_right', 'Dörr sovrum',    'Bedroom door')
  ) AS d(x, y, rot, symbol, name_sv, name_en);
END;
$$;

-- Wire into the canonical seed so per-user demos + rebuilds get the plan.
-- (Appended call; seed_demo_content body otherwise untouched — the room
-- legacy shapes it creates are exactly what seed_demo_floor_plan copies.)
...
BEGIN
  ...
  -- Quote line items — mirror the tasks (customer price ex VAT, per-task ROT)
  -- so the accepted quote renders a coherent breakdown instead of empty (0 kr).
  INSERT INTO quote_items (quote_id, description, quantity, unit, unit_price, is_rot_eligible, rot_deduction, sort_order) VALUES
    (v_quote_id, CASE WHEN v_is_en THEN 'Preparation and protection' ELSE 'Förberedelse och skydd' END,        1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 3000,  true, 990,  1),
    (v_quote_id, CASE WHEN v_is_en THEN 'Demolition and strip-out' ELSE 'Rivning och demontering' END,          1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 6000,  true, 1650, 2),
    (v_quote_id, CASE WHEN v_is_en THEN 'Wall spackling' ELSE 'Spackling av väggar' END,                        1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 7000,  true, 2310, 3),
    (v_quote_id, CASE WHEN v_is_en THEN 'Wall & ceiling painting' ELSE 'Målning väggar & tak' END,              1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 12000, true, 2805, 4),
    (v_quote_id, CASE WHEN v_is_en THEN 'Feature wall wallpapering' ELSE 'Tapetsering fondvägg' END,            1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 6000,  true, 495,  5),
    (v_quote_id, CASE WHEN v_is_en THEN 'Bathroom waterproofing' ELSE 'Tätskikt badrum' END,                    1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 9000,  true, 1320, 6),
    (v_quote_id, CASE WHEN v_is_en THEN 'Bathroom tiling' ELSE 'Kakelsättning badrum' END,                      1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 28000, true, 3300, 7),
    (v_quote_id, CASE WHEN v_is_en THEN 'Countertop and backsplash' ELSE 'Bänkskiva och stänkskydd' END,        1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 22000, true, 2970, 8),
    (v_quote_id, CASE WHEN v_is_en THEN 'Floor sanding and lacquering' ELSE 'Slipning och lackning av golv' END, 1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 18000, true, 1980, 9),
    (v_quote_id, CASE WHEN v_is_en THEN 'Outlets and lighting' ELSE 'Eluttag och belysning' END,                1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 12000, true, 660,  10),
    (v_quote_id, CASE WHEN v_is_en THEN 'Install trim and moldings' ELSE 'Montering av lister' END,             1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 7000,  true, 1155, 11),
    (v_quote_id, CASE WHEN v_is_en THEN 'Final cleaning' ELSE 'Slutstädning' END,                               1, CASE WHEN v_is_en THEN 'pcs' ELSE 'st' END, 4000,  true, 1485, 12);
  ...
  -- Floor plan with walls + doors derived from the room polygons (SP2).
  PERFORM seed_demo_floor_plan(p_project_id, p_language);

END;
$$;

-- Backfill the PUBLIC demo now (its "Floor Plan 1" is normalized + filled).
