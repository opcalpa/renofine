...
function buildSystemPrompt(language: string, rooms: RoomCtx[], tasks: TaskCtx[], memories: MemoryCtx[], members: MemberCtx[], userType?: string | null): string {
  ...
  const roleSection = userType
    ? (userType === "contractor"
        ? `\nUSER ROLE: contractor (a professional). You MAY propose open_feature to prepare a quote, invoice or ÄTA (change/additional work beyond the original quote) — Renaida cannot fill a full quote inline, so she opens the right builder (prefilled where possible) for the user to review and send. Never auto-send. You MAY also propose set_default_rate when they state their own standing hourly rate or markup.\n`
...
- { "type": "open_feature", "feature": "new_quote"|"new_invoice"|"new_ata", "label": "<short label in ${langName} naming what opens, e.g. Ny offert / Ny faktura / Ny ÄTA>" } — a POINTER, not a data change: opens the right screen (prefilled where possible) for something Renaida should not fill inline. Contractor-only; used for quote/invoice/ÄTA CREATION.
- { "type": "set_default_rate", "field": "hourly_rate"|"markup_percent"|"material_markup_percent", "value": <number> } — updates the contractor's own DEFAULT rates on their profile (the source of truth for estimates/quotes). Contractor-only.
...
- DEFAULT RATES (set_default_rate, contractor-only): statements about the builder's OWN standing rates — "mitt timpris är 640", "jag brukar ta 640 kr i timmen" → { field: "hourly_rate", value: 640 }; "jag har 15% påslag på arbete/UE" → { field: "markup_percent", value: 15 }; "10% påslag på material" → { field: "material_markup_percent", value: 10 }. This changes their PROFILE DEFAULTS, not a task: a price/budget for a SPECIFIC task ("höj budgeten för målningen till 15000") is update_task, and an hourly rate said while logging time is log_time context — NEVER set_default_rate. If USER ROLE is homeowner, do NOT propose it.
- FEATURE GUIDANCE (open_feature): creating a quote, invoice or ÄTA is not filled in inline — "förbered/gör en offert" → new_quote (offert); "skapa en faktura till kunden" → new_invoice (faktura); an ÄTA (ändrings- och tilläggsarbete — extra/ändrat arbete UTÖVER den ursprungliga offerten: "gör en ÄTA", "lägg till ett tilläggsarbete", "kunden vill ha extra arbete utöver offerten", "ändringsarbete") → new_ata. Emit open_feature { feature, label }. This is a CONTRACTOR action: if USER ROLE is homeowner, do NOT propose it. A quote/invoice the user RECEIVED (a document to upload) is NOT open_feature — that is the document flow. open_feature never writes data; it only opens a screen.
...
function normalizeProposals(
  ...
  for (const p of raw) {
    ...
    switch (action.type) {
      ...
      case "open_feature": {
        const FEATURES = new Set(["new_quote", "new_invoice", "new_ata"]);
        const feature = typeof action.feature === "string" ? action.feature : "";
        if (!FEATURES.has(feature)) {
          toUnknown("Okänd funktion att öppna");
          break;
        }
        // Defense in depth: quote/invoice creation is a contractor action —
        // never route a homeowner there (the route is contractor-gated too).
        if (userType && userType !== "contractor") {
          toUnknown("Den funktionen är för proffs");
          break;
        }
        const label = typeof action.label === "string" && action.label.trim() ? action.label.trim() : feature;
        out.push({ id, summary, confidence, action: { type: "open_feature", feature, label } });
        break;
      }
      case "set_default_rate": {
        const RATE_FIELDS = new Set(["hourly_rate", "markup_percent", "material_markup_percent"]);
        const field = typeof action.field === "string" ? action.field : "";
        const value = typeof action.value === "number" && Number.isFinite(action.value) ? action.value : NaN;
        // Sanity bounds: an hourly rate is 1–20 000 kr/h, a markup 0–100 %.
        const inBounds = field === "hourly_rate" ? value > 0 && value <= 20000 : value >= 0 && value <= 100;
        if (!RATE_FIELDS.has(field) || Number.isNaN(value) || !inBounds) {
          toUnknown("Kunde inte tolka satsen");
          break;
        }
        // Contractor-only: a homeowner has no builder rates to update.
        if (userType && userType !== "contractor") {
          toUnknown("Standardsatser är för proffs");
          break;
        }
        out.push({ id, summary, confidence, action: { type: "set_default_rate", field, value } });
        break;
      }
      case "unknown": {
