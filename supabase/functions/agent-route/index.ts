...
function buildSystemPrompt(language: string, rooms: RoomCtx[], tasks: TaskCtx[], memories: MemoryCtx[], members: MemberCtx[], userType?: string | null): string {
  ...
  const roleSection = userType
    ? (userType === "contractor"
        ? `\nUSER ROLE: contractor (a professional). You MAY propose open_feature to prepare a quote, invoice or ÄTA (change/additional work beyond the original quote) — Renaida cannot fill a full quote inline, so she opens the right builder (prefilled where possible) for the user to review and send. Never auto-send.\n`
